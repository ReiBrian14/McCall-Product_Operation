...
if (isset($_POST['save'])) {
    ...
    $dataSudahAda = false;
    $data = mysqli_query($koneksi, "SELECT * FROM penilaian WHERE id_penilaian='$id_akhir'");

    // menghitung jumlah data yang ditemukan
    $cek = mysqli_num_rows($data);
 
    if ($cek > 0) {
        $update = $koneksi->query("update penilaian set hasil = '$nilai_akhir',kelayakan='$kategori_akhir' where id_penilaian = $id_akhir");
